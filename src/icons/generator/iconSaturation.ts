...
const addFilterElement = (svg: string, value: number) => {
    ...
    const filterElement = `<filter id="saturation"><feColorMatrix in="SourceGraphic" type="hueRotate" values="-10" result="hue" /><feColorMatrix in="hue" type="saturate" values="${value}" result="sat" /></filter>`;
    if (pattern.test(svg)) {
...
export const validateSaturationValue = (saturation: number) => {
    return saturation !== null && saturation >= 0;
};
