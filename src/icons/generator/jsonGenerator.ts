...
export const generateIconConfigurationObject = (
  ...
  const languageIconDefinitions = getLanguageIconDefinitions(
    languageIcons,
    iconConfig,
    options,
  );
  const fileIconDefinitions = getFileIconDefinitions(
    fileIcons,
    iconConfig,
    options,
  );
  const folderIconDefinitions = getFolderIconDefinitions(
    folderIcons,
    iconConfig,
    options,
  );
...
export const createIconFile = (
  ...
  if (updatedConfigs) {
    if (
      updatedConfigs.opacity !== undefined &&
      !validateOpacityValue(updatedConfigs.opacity)
    ) {
      throw Error('POP! Icons: Invalid opacity value!');
    }
    if (
      updatedConfigs.saturation !== undefined &&
      !validateSaturationValue(updatedConfigs.saturation)
    ) {
      throw Error('POP! Icons: Invalid saturation value!');
    }
  }
  ...
  if (updatedConfigs && updatedConfigs.folders) {
    if (typeof updatedConfigs.folders.color !== 'undefined') {
      if (!validateHEXColorCode(updatedConfigs.folders.color)) {
        throw Error('POP! Icons: Invalid folder color value!');
      }
    }
  }
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(iconsPath, iconJsonName),
