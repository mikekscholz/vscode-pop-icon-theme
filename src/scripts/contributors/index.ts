...
const fetchContributors = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(requestOptions, res => {
      const { nextPage, lastPage, prevPage } = res.headers.link
        ? parseLinkHeader(res.headers.link.toString())
        : {
            nextPage: undefined,
            lastPage: [undefined, 1],
            prevPage: undefined,
          };
      console.log(
        '> POP! Icon Theme:',
      ...
      res.on('end', () => {
        try {
          ...
          resolve({
            contributorsOfPage,
            nextPage: nextPage ? nextPage[1] : undefined,
          });
        } catch (error) {
          reject(error);
        }
 ...
 */
const createLinkedImages = (
  contributors: Contributor[],
  imageSize: number = 40,
) => {
  const linkList = contributors
    .map(c => {
...
const updateContributors = async (config: ContributorsConfig) => {
  ...
  const getContributorsSectionPattern = new RegExp(
    /(\[\/\/\]:\s#contributors\s\(start\))([\s\S]*)(\[\/\/\]:\s#contributors\s\(end\))/,
  );
  const updatedContent = readmeContent.replace(
    getContributorsSectionPattern,
    `$1\n${images}\n\n$3`,
  );
